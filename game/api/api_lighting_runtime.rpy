...
init -100 python:
    ...
    if not hasattr(store, '_lighting_next_restart'):
        store._lighting_next_restart = 0.0

    def _lighting_request_refresh(force=False, cooldown=0.12):
        try:
            if getattr(store, 'lighting_editor_open', False) and '_lighting_editor_queue_restart' in globals():
                if force:
                    _lighting_editor_queue_restart(0.0)
                else:
                    _lighting_editor_queue_restart(cooldown)
                return True
        except Exception:
            pass
        try:
            if getattr(store, 'shader_editor_open', False) and '_shader_editor_queue_restart' in globals():
                _shader_editor_queue_restart(force=force)
                return True
        except Exception:
            pass
        if force:
            store._lighting_next_restart = time.time() + cooldown
            renpy.restart_interaction()
            return True
        now = time.time()
        next_allowed = getattr(store, '_lighting_next_restart', 0.0)
        if now >= next_allowed:
            store._lighting_next_restart = now + cooldown
            renpy.restart_interaction()
        return True

    # Lighting editor toggle (kept lightweight but functional).
    def toggle_lighting_editor(force=None):
        try:
            if force is None:
                store.lighting_editor_open = not getattr(store, 'lighting_editor_open', False)
            else:
                store.lighting_editor_open = bool(force)
            _lighting_request_refresh(force=True)
            return store.lighting_editor_open
        except Exception:
            return False

    # Runtime lighting public surface — syncs dynamic_lights and uniforms.
    def lights_runtime_apply_lights(lights, replace=True):
        try:
            existing = list(getattr(store, 'dynamic_lights', []) or [])
            incoming = []
            for entry in (lights or []):
                if isinstance(entry, dict):
                    incoming.append(dict(entry))

            if replace:
                merged = incoming
            else:
                merged = existing[:]
                id_index = {str(e.get('id')): i for i, e in enumerate(merged) if 'id' in e}
                for entry in incoming:
                    key = str(entry.get('id')) if 'id' in entry else None
                    if key and key in id_index:
                        merged[id_index[key]] = entry
                    else:
                        merged.append(entry)

            store.dynamic_lights = merged
            store.lights_state["active"] = list(merged)
            store.lights_state["enabled"] = bool(merged)
