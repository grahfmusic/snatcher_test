# YAML IO API for Snatchernauts Framework
# Ren'Py compliant wrapper for YAML functionality
...
init -1 python:
    ...
    packages_path = os.path.join(renpy.config.gamedir, "python-packages")
    if packages_path not in sys.path:
        sys.path.insert(0, packages_path)
    
    # Try to import YAML, but make it optional for compatibility
    yaml_module = None
    try:
        import yaml
        from yaml import SafeLoader, SafeDumper
        yaml_module = yaml
        print("[YAML API] YAML module loaded successfully")
    except ImportError:
        print("[YAML API] YAML module not available - YAML functionality disabled")
    
    class CustomYAMLDumper(SafeDumper if yaml_module else object):
    ...
    class YAMLIOService:
        ...
        def load_yaml(self, file_path):
            ...
            """
            if not yaml_module:
                print("[YAML API] YAML module not available, cannot load file")
                return {}
            ...
            try:
                ...
                # Parse YAML
                data = yaml_module.load(content, Loader=SafeLoader)
                return data if data is not None else {}
                
            except Exception as e:
                print(f"[YAML API] Load error for {file_path}: {e}")
                return {}
        
        def save_yaml_if_changed(self, file_path, data, force=False):
            """
            ...
            """
            if not yaml_module:
                print("[YAML API] YAML module not available, cannot save file")
                return False
            ...
            try:
                ...
                try:
                    ...
                    print(f"[YAML API] Saved: {file_path}")
                    return True
                ...
                print(f"[YAML API] Save error for {file_path}: {e}")
                return False
        
        def _generate_stable_yaml(self, data):
            """
        ...
        def compute_diff(self, old_data, new_data):
            """
    ...
    # Convenience functions for Ren'Py
    def load_yaml(file_path):
        """Load YAML file. Convenience wrapper."""
        return get_yaml_service().load_yaml(file_path)

    def save_yaml_if_changed(file_path, data, force=False):
    ...
    def yaml_available():
        ...
        return yaml_module is not None
