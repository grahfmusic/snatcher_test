...
define config.lint_hooks = []
define config.missing_label_callback = None

# Disable some problematic features during lint
init python:
    # Configure lint-safe environment
    import os
    if "--lint" in sys.argv or os.environ.get("RENPY_LINT"):
        # Disable problematic features during lint
        config.developer = False
        config.console = False

## Skipping configuration ####################################################
##
## Disable all skipping functionality globally in the framework

# Disable all forms of skipping
define config.allow_skipping = False
define config.fast_skipping = False
define config.skipping = None
define config.skip_delay = 0
define config.skip_indicator = False

# More comprehensive skipping configuration
init python:
    # Remove all skipping-related keymaps completely
    config.keymap = dict(config.keymap)  # Make it mutable
    config.keymap["skip"] = []
    config.keymap["toggle_skip"] = []
    config.keymap["fast_skip"] = []
    if "skip_unseen" in config.keymap:
        config.keymap["skip_unseen"] = []
    if "skip_seen" in config.keymap:
        config.keymap["skip_seen"] = []
    
    # Disable skip-related preferences
    config.skipping = False
    
    # Override skip notification
    def null_skip(*args, **kwargs):
        renpy.notify("Skipping is disabled in this framework")
        return
